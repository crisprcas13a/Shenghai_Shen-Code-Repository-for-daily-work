...
class Curve:
    ...
    def get_b(self, r, i, t):
        if r==0:
            return self.np_points[i]
        else: 
            return (1-t)*self.get_b(r-1, i, t) + t*self.get_b(r-1, i+1, t)
    ...
    def draw_bezier(self, canva):
        ...
        self.bezier = np.asarray([])
        n = len(self.points) - 1
        self.np_points = np.asarray(self.points)
        for t in np.arange(0, 1+self.interval, self.interval):
            # get b^n(t) recursively
            b_t = self.get_b(n, 0, t)
