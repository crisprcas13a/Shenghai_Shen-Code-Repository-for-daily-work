    ...
    def draw_Bspline(self, canva):
        ...
        if degree == 1:
            for i in range(0, num_points):
                ...
                x = 0.5*self.points[i][0] + 0.5*self.points[(i+1)%num_points][0]
                y = 0.5*self.points[i][1] + 0.5*self.points[(i+1)%num_points][1]
                point = (x, y)
                new_points.append(point)
                t_point = canva.create_oval(x-4, y-4, x+4, y+4, fill="#000000")
            ...
            for i in range(0, num_points):
                x = 0.75*self.points[i][0] + 0.25*self.points[(i+1)%num_points][0]
                y = 0.75*self.points[i][1] + 0.25*self.points[(i+1)%num_points][1]
                point = (x, y)
                new_points.append(point)
                t_point = canva.create_oval(x-4, y-4, x+4, y+4, fill="#000000")
                new_t_points.append(t_point)
                x = 0.25*self.points[i][0] + 0.75*self.points[(i+1)%num_points][0]
                y = 0.25*self.points[i][1] + 0.75*self.points[(i+1)%num_points][1]
                point = (x, y)
                new_points.append(point)
                t_point = canva.create_oval(x-4, y-4, x+4, y+4, fill="#000000")
            self.points = new_points
            ...
            for i in range(0, num_points):
                x = 1/8*self.points[(i-1+num_points)%num_points][0] + 6/8*self.points[i][0] + 1/8*self.points[(i+1)%num_points][0]
                y = 1/8*self.points[(i-1+num_points)%num_points][1] + 6/8*self.points[i][1] + 1/8*self.points[(    i+1)%num_points][1]
                point = (x, y)
                new_points.append(point)
                t_point = canva.create_oval(x-4, y-4, x+4, y+4, fill="#000000")
                new_t_points.append(t_point)
            ...
            print("Sorry, only support degree 1, 2, 3.")
...
def main():
    global degree
    degree = input("Please input a positive integer in [1, 3] as the degree\n")
    degree = int(degree)
