...
class Curve:
    ...
    def __init__(self):
        ...
        # At most 128 points, we can edit this number to set the limit
        self.C = np.zeros((self.maxPointNum, self.maxPointNum))
        self.compute_C()
        # Set the t's interval as this number
        self.interval = interval

    # This function is to draw lines between points of the Bezier curve
    ...
    def draw_bezier(self, canva):
        ...
        for t in np.arange(0, 1+self.interval, self.interval):
            ...
            for i in range(0, n+1):
                B = np.append(B, self.C[n][i]*(t**i)*((1-t)**(n-i)))
            b_t = np.sum(B.reshape(n+1,1)*np_points, axis=0)
    ...
    # At the first of this program, compute the combinations numbers and store them.
    # No need to compute them during drawing cureve
    def compute_C(self):
       self.C[0][0] = 1
       for n in range(1, 100):
           self.C[n][0] = 1
           for i in range(1, n+1):
               self.C[n][i] = self.C[n-1][i-1] + self.C[n-1][i]
    ...
    def move_point(self, x, y, canva):
        ...
        # each time moving a point, draw the bezier curve again
