...
class Curve:
    ...
    def d(self, r, j, t):
        if r==0:
            return np.asarray(self.points[j])
        else:
            w = (t-self.knots[j])/(self.knots[j+self.degree-r+1]-self.knots[j])
            return (1-w)*self.d(r-1, j-1, t) + w*self.d(r-1, j, t)
    ...
    def draw_b_spline(self, canva):
        ...
        for i in range(k, n+1):
            for t in np.arange(self.knots[i], self.knots[i+1], self.interval):
                for j in range(i, i+1):
                    b_t = self.d(self.degree, j, t)
